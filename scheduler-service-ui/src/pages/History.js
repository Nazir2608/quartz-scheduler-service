...
import { 
  Container, Typography, Paper, Box, Chip, Fade, Alert,
  Table, TableBody, TableCell, TableContainer, TableHead, 
  TableRow, TablePagination, IconButton, Tooltip,
  TextField, Toolbar, InputAdornment, Stack, Button
} from "@mui/material";
import { 
  History as HistoryIcon, AccessTime, Work, Search as SearchIcon,
  Refresh as RefreshIcon, Info as InfoIcon,
  Replay as ReplayIcon, Delete as DeleteIcon
} from "@mui/icons-material";
import { getHistoryJobs, deleteJob } from "../api/jobsApi";
...
const History = () => {
  ...
  const fetchHistoryJobs = async () => {
    try {
      ...
      setError("Failed to fetch history jobs. Please try again.");
      console.error("Error fetching history jobs:", err);
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <Container maxWidth={false} sx={{ mt: 10, ml: '280px', mr: 4, width: 'calc(100% - 312px)' }}>
        <Box display="flex" justifyContent="center" alignItems="center" minHeight="60vh">
          <HistoryIcon sx={{ fontSize: 60, color: 'text.secondary', animation: 'spin 2s linear infinite' }} />
        </Box>
      </Container>
    );
  }
        ...
        <Box>
          ...
          >
            <Box display="flex" alignItems="center" gap={3} position="relative" zIndex={1}>
              <HistoryIcon sx={{ fontSize: 48, filter: 'drop-shadow(0 2px 4px rgba(0,0,0,0.3))' }} />
              <Box>
                <Typography variant="h3" component="h1" fontWeight="bold" sx={{ textShadow: '0 2px 4px rgba(0,0,0,0.3)' }}>
                  Job History
                </Typography>
                <Typography variant="h6" sx={{ opacity: 0.9, fontWeight: 400 }}>
                  Track completed, failed, and cancelled jobs
                </Typography>
              </Box>
            </Box>
            ...
            <Box display="flex" gap={4} mt={3}>
              <Box display="flex" alignItems="center" gap={1}>
                ...
                <Typography variant="body2">
                  <strong>Total History:</strong> {jobs.length}
                </Typography>
              </Box>
              <Box display="flex" alignItems="center" gap={1}>
              ...
              <Table stickyHeader sx={{ minWidth: 650 }}>
                <TableHead sx={{ backgroundColor: 'action.hover' }}>
                  <TableRow>
                    ...
                    <TableCell sx={{ fontWeight: 'bold' }}>Completed Time</TableCell>
                        ...
                        <Stack direction="row" spacing={1} justifyContent="center">
                          <Tooltip title="View Details">
                            <IconButton
                              size="small"
                              color="primary"
                              onClick={() => navigate(`/details/${job.jobId}`, { state: job })}
                            >
                          ...
                          <Tooltip title="Re-run Job">
                            <IconButton
                              size="small"
                              color="success"
                              onClick={() => navigate('/add', { state: { rerunJob: job } })}
                            >
            ...
            {filteredJobs.length === 0 && (
              ...
              >
                ...
                <Typography variant="h6" color="text.secondary" gutterBottom>
                  {searchTerm ? 'No jobs match your search' : 'No job history available'}
                </Typography>
                <Typography variant="body2" color="text.secondary">
                  {searchTerm ? 'Try adjusting your search criteria' : 'Job history will appear here'}
                </Typography>
              </Box>
            )}
